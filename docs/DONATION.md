...
### 5.4 捐赠记录（已决：完全不记录）
- **不打任何捐赠记录、不落库**：webhook 收到 donation 仅跳过，不做任何 D1 写入。
- 隐私最优，与「本地优先」定位一致。仅本地「💜 已支持」标记存于用户浏览器 localStorage。
...
- 打赏全程匿名：`custom_id` 为随机 uuid，不关联账号或健康数据；不落库。
...
1. **捐赠记录**：✅ **完全不记录**（最隐私，webhook 仅跳过、不落库）。
