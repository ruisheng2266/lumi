...
### 🟢 v0.5 — 洞察深化 + 多语言 + 围绝经期（全免费）
| 功能 | 说明 | 依据 |
| --- | --- | --- |
| **① 本地多年趋势 & 相关性** | 端上长期趋势图、症状-阶段相关性（培养使用习惯，为 Plus 蓄水池） | PRD V2 计划 |
| **② 围绝经期专属工具** | 围绝经期/更年期阶段标记、潮热/盗汗等专属症状与模式洞察（Flo/Clue 有、经期助手无的 gap） | 竞品差异化机会 |
| **③ i18n 扩展 ja / ko / zh-TW** | 在可插拔架构上新增三种语言文件 | PRD §6.5.2 |
| **④ 备孕模式基础** | BBT 基础体温录入（本地），为未来曲线打底 | PRD V2 备孕 |
| **⑤ 中立健康科普** | 应用内轻量科普内容（不作诊断），强化"诚实有帮助" | PRD §10.3 文案原则 |
...
| --- | --- | --- | --- |
| v0.4 / v0.5 | Free 持续变厚（导入、特殊场景、围绝经期、多语言、a11y） | — | 零变现，积累口碑基数 |
...
- [ ] **v0.4 优先级**：① 竞品导入 vs ② 特殊场景，哪个先做？（建议先做①，获客杠杆最强）
