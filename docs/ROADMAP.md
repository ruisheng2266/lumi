...
> - **深色模式品牌色文字对比度修复（WCAG AA）**：根因是 Tailwind 固定深色字落在深色底仅 ~3:1；仅在 `.dark` 下改用随主题的更亮同色系，实测深色 6.2–6.8:1、浅色 4.5–6.8:1；108 测试 + 构建通过。
> - **已知次要残留**：主按钮深色模式 `hover` 态白字 ≈ 2.95:1（瞬时态，待收尾，不影响常态对比度）。
