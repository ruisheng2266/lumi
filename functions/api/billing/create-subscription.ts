 ...
 * Phase 3：创建 Plus 年付订阅（PayPal Subscriptions API）。
 *
 *  POST /api/billing/create-subscription  → { subscriptionId, approveUrl }
 *  需预先在 PayPal 后台创建 Plan，ID 存入 PAYPAL_PLUS_PLAN_ID。
...
export const onRequestPost: Handler = async (context) => {
  try {
    ...
    if (!cfg.plusPlanId) return json({ error: 'paypal_plus_plan_missing' }, { status: 503 });

    const { subscriptionId, approveUrl } = await createSubscription(cfg, {
      planId: cfg.plusPlanId,
      customId: userId,
    });
    return json({ subscriptionId, approveUrl });
  } catch (err) {
